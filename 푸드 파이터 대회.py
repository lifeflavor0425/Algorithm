...
def solution(food):
    food_arr = []
    food.pop(0)  # 물 배제
    food_arr = [index for index, fod in enumerate(food, 1) for cnt in range(fod // 2)]
    food_arr.extend(sorted(food_arr, reverse=True))
    food_arr.insert(len(food_arr) // 2, 0) # 물 추가
    answer = "".join(map(lambda x: str(x), food_arr))
    print(answer)
    return answer
