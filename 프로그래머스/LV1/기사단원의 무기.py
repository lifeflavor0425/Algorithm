...
def solution(number, limit, power):
    # 1. number를 반복하면서 약수의 개수 구하기 => dict
    tmp_dict = {"1": 1}
    for player in range(2, number + 1):
        tmp_dict[f"{player}"] = len(getMyDivisor(player))
    # 2. 약수의 개수 가 limin 보다 큰지 비교
    for key in tmp_dict:
        if tmp_dict[key] > limit:
            tmp_dict[key] = power

    print(sum(list(tmp_dict.values())))
    return sum(list(tmp_dict.values()))
