...
def solution(board, moves):
    ...
    for i in range(len(board)):
        ...
        tmp_all.append(tmp)
    board = tmp_all
    board_no_zero = delete_zero(board)
    # 1. 바구니 stack 배열 생성
    stack = list()
    # 2. moves 를 반복하면서 board에 뒷 인덱스 부터 바구니 stack에 삽입
    for position in moves:
        if not board_no_zero[position - 1]:
            continue
        stack.append(board_no_zero[position - 1][-1])
        board_no_zero[position - 1][-1] = 0
        board_no_zero = delete_zero(board_no_zero)
        # 3. 바구니 스택에 전 값과 현 값이 같으면 두개의 값을 없앤다.
        # 4. 없애면 answer를 2씩 증가시키고, 반환한다.
        if len(stack) > 1:
            if stack[-1] == stack[-2]:
                answer += 2
                stack.pop(-1)
...
def delete_zero(arr):
    ...
    for i in range(len(arr)):
        board_no_zero.append(list(filter(lambda x: x > 0, arr[i])))
    return board_no_zero
